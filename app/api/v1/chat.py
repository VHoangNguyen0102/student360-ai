...
Routes to FinanceAgent (Phase 1: 6-Jars context).
Phase C+: switch to OrchestratorAgent based on context_hint.
"""
...
def _synthesise_reply_from_tools(messages: list) -> str | None:
    ...
    # If we can detect the tool name and it's not jar stats, don't synthesize.
    if tool_name is not None and tool_name != "get_jar_statistics":
        return None
